...
ClientDigitalTwin::Door::Door(std::string name, std::string type,
  ...
  this->isOpenThisDoor = static_cast<bool>(value);
  this->oldStateOpen = this->isOpenThisDoor;
  this->parsingOption();
}
...
void ClientDigitalTwin::Door::ShowWindow(const Camera3D &camera) {
  ...
  std::string openText =
      this->type + ": " + (this->isOpenThisDoor ? "open" : "close");
  GuiLabel(
  ...
  if (this->isAlarm && this->isOpenThisDoor) {
  ...
  // Toggle door
  std::string btnText = this->isOpenThisDoor ? "Close" : "Open";
  if (GuiButton({this->windowRect.x + 143.0f, this->windowRect.y + 102.0f,
                 100.0f, 30.0f},
                btnText.c_str())) {
    this->isOpenThisDoor = !this->isOpenThisDoor;
  }
  if (this->isOpenThisDoor != this->oldStateOpen && isControl) {
    this->makeValue();
...
void ClientDigitalTwin::Door::makeValue() {
  this->sendValue["value"] = static_cast<float>(this->isOpenThisDoor);
}